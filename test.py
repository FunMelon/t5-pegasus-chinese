...
def load_data(filename):
    """加载数据，CSV格式：id, content"""
    D = []
    with open(filename, encoding='utf-8') as f:
        reader = csv.reader(f, delimiter='|')
        for row in reader:
            if len(row) == 2:
                D.append((row[0], row[1]))  # id, content
    return D
...
def generate(test_data, model, tokenizer, args):
    ...
    with open(args.result_file, 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f, delimiter='|')  # 使用|分隔符
        writer.writerow(['id', 'ret'])  # 写入标题行
        model.eval()
        
        for feature in tqdm(test_data):
            raw_data = feature['raw_data']
            content = {k: v for k, v in feature.items() if k not in ['raw_data', 'title', 'id']} 
            ...
            # 遍历生成的摘要，每个摘要对应一个ID，逐行写入
            for idx, summary in enumerate(gen):
                writer.writerow([feature['id'][idx], summary])  # 每个摘要单独一行，ID对应摘要
            gens.extend(gen)
...
def init_argument():
    ...
    parser.add_argument('--test_data', default='./data/test_dataset.csv')  # 修改为test.csv
    parser.add_argument('--result_file', default='./test_result.csv')  # 输出为test_result.csv
